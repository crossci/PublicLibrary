...
inline int BASE64_Encode( const unsigned char* inputBuffer, int inputCount, std::string& outputBuffer )
{
	int i;
	unsigned char b0, b1, b2;

	if( (inputBuffer == 0) || (inputCount < 0) )
	{
		return -1;	// 参数错误
	}
	...
	//if( outputBuffer != 0 )
	{
		for( i = inputCount; i > 0; i -= 3 )
		{
			...
			{	// 将3字节数据转换成4个ASCII字符
				...
				outputBuffer.append(&DATA_BIN2ASCII[b0 >> 2],1);
				outputBuffer.append(&DATA_BIN2ASCII[((b0 << 4) | (b1 >> 4)) & 0x3F],1);
				outputBuffer.append(&DATA_BIN2ASCII[((b1 << 2) | (b2 >> 6)) & 0x3F],1);
				outputBuffer.append(&DATA_BIN2ASCII[b2 & 0x3F],1);
			}
			else
			{
				b0 = *inputBuffer++;
				...
				outputBuffer.append(&DATA_BIN2ASCII[b0 >> 2],1);
				outputBuffer.append(&DATA_BIN2ASCII[((b0 << 4) | (b1 >> 4)) & 0x3F],1);
				outputBuffer.append((i == 1) ? "=" : &DATA_BIN2ASCII[(b1 << 2) & 0x3F],1);
				outputBuffer.append("=");
			}
		} // End for i

		outputBuffer.append(outputBuffer.length(),'/0');
	}
...
inline int BASE64_Decode( const char* inputBuffer, int inputCount, unsigned char* outputBuffer )
{
	int i, j;
	unsigned char b[4];
	char ch;
